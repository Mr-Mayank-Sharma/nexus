...
export default function TaskQueuesPage() {
  ...
  const getPriorityColor = (p: string) => {
    switch (p) {
      ...
      case 'medium': return 'bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400'
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2.5"><ClipboardList className="w-7 h-7 text-primary-500" /> Task Queues</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review and resolve exception tasks</p>
        </div>
          ...
          {(['all', 'high', 'medium', 'low'] as const).map(p => (
            <button key={p} onClick={() => setPriorityFilter(p)}
              className={clsx('px-3 py-1.5 text-xs font-medium rounded-md capitalize',
                priorityFilter === p ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300')}>
              {p}
            </button>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="enterprise-card p-12 text-center">
          ...
          <p className="font-medium text-gray-500 dark:text-gray-400">No pending tasks in this queue</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filteredItems.map(item => (
            <div key={item.id} className="enterprise-card p-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-all">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  <div className={clsx('w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5',
                    activeTab === 'swap' && 'bg-blue-50 dark:bg-blue-900/20 text-blue-600',
                    activeTab === 'bad-address' && 'bg-amber-50 dark:bg-amber-900/20 text-amber-600',
                    activeTab === 'fraud' && 'bg-red-50 dark:bg-red-900/20 text-red-600',
                    activeTab === 'hold' && 'bg-purple-50 dark:bg-purple-900/20 text-purple-600',
                  )}>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{item.orderNumber}</span>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-2.5 mt-1.5">
                      <p className="text-xs font-medium text-gray-700 dark:text-gray-300 flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3 text-amber-500" />
                        {item.reason}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.details}</p>
                    </div>
                  </div>
                </div>
