  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2.5"><ShoppingBag className="w-7 h-7 text-primary-500" /> BOPIS</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Buy Online, Pick Up In Store</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="enterprise-card p-12 text-center">
          ...
          <p className="font-medium text-gray-500 dark:text-gray-400">No {activeTab} items</p>
        </div>
      ) : (
                  ...
                  {order.items && order.items.length > 0 && (
                    <div className="mt-3 ml-13 pl-13 flex gap-3">
                      {order.items.slice(0, 3).map((item: any, i: number) => (
                        <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-xs text-gray-600 dark:text-gray-400">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4">
                {filteredProducts.map(product => (
                  <button key={product.id} onClick={() => navigate(`/products/${product.id}`)}
                    className="group text-left p-3 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-sm transition-all">
                    <div className="w-full aspect-square rounded-lg bg-gray-100 dark:bg-gray-700 mb-2 flex items-center justify-center text-gray-400 dark:text-gray-500">
                      <Store className="w-8 h-8" />
                    </div>
                    <p className="text-xs font-medium text-gray-900 dark:text-gray-100 truncate">{product.name}</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 truncate">{product.sku}</p>
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-sm font-bold text-primary-600 dark:text-primary-400">${product.price?.toFixed(2) || '0.00'}</span>
                      <span className={clsx('text-[10px] font-medium',
                        (product.stock || 0) > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500'
                      )}>
