...
The **skeleton is complete, the locks work, and the practice exams pass (561/561)**. The job now is **trust**: giving the AI real food to cook with and wiring the last simulated doors to the real world.
...
| **Migrations** | 54 Flyway migrations (`V1`…`V54`) — schema, seed data, AI `metrics_source`, shipping-label source, multi-client billing |
...
| **Testing** | Backend: **561 unit tests, 0 failures, 0 errors** (`mvn test` green). Frontend: Playwright smoke-tested on live dev stack |
...
- Inbound (PO receiving) and outbound processing
...
- **WES-lite**: task queue + automation commands (`move`, `putaway`, …) with honest `simulated:true` flags for non-wired equipment
...
- Purchase orders (subtotal/tax/total computation), vendors (supplier codes, tax IDs, status), replenishment rules; warehouse zones/aisles/bins, labor tasks, automation config (AGV/ASRS), IoT/telemetry hooks

### 2.6 Finance, billing & 3PL 💰
- **Invoicing**: invoice creation (subtotal/tax/discount/shipping → total), payments (`recordPayment` with settlement + over-payment rejection), **refunds + credit memos**, invoice summary (outstanding / overdue / paid-this-month) — unit-tested
...
- **Backend test suite** — expanded from 501 → **561 green tests** with new suites for ATP, yard/dock, procurement, invoicing, EDI (incl. dry-run/upload), BOPIS pickup lifecycle, billing, rate cards, waves and task-queue.
...
- **Testing**: 561 backend unit tests green; Playwright smoke suite on the live dev stack (rate cards, billing statements, client portal) with 0 console errors.
...
| Self-optimizing warehouse | Wave plan/release + task queue + automation commands exist | Closed-loop slotting/labor optimization (G8) |
| Trustworthy AI platform | Model registry, experiments, drift, fallbacks, audit | Real training data → **no production model yet** (G3) |
| One-touch integrations | 15+ connectors + EDI 850/856/810 + iPaaS-lite | Depth (auth refresh, idempotent sync state) |
| 3PL-grade billing | Rate cards + statements + read-only client portal | Self-serve portal, credit/payment workflows (G9) |
| GA quality | 54 migrations, 66 controllers, 89 pages, 561 tests green | Stub→real provider wiring (G4), real ML data (G3) |
