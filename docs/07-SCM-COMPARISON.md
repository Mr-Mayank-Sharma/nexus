...
| 12 | **Nexus (us)** | — | Omnichannel OMS + WMS + 3PL + AI | Mid-market omnichannel | Self-built, 72 commits, **561 tests green**, RF + 3PL billing shipped |
...
| Warehouse execution system (WES) / automation (AGV/ASRS/robotics) | 9 🟩 | 3 🟥 | 8 🟩 | 8 🟩 | 8 🟩 | 9 🟩 | 10 🟩 | 5 🟨 | 4 🟥 | 6 🟨 | 5 🟨 | **6 🟨** |
| Mobile RF / handheld warehouse app | 9 🟩 | 4 🟥 | 8 🟩 | 8 🟩 | 8 🟩 | 9 🟩 | 9 🟩 | 9 🟩 | 6 🟨 | 8 🟩 | 5 🟨 | **7 🟨** |
...
| Finance: invoicing, payments, accounting link | 6 🟨 | 4 🟥 | 7 🟨 | 9 🟩 | 10 🟩 | 5 🟨 | 4 🟥 | 9 🟩 | 7 🟨 | 6 🟨 | 9 🟩 | **7 🟨** |
...
| 7 | **NEXUS (us)** | **7.5** | **6.9** | **7.2** | **7.0** | **7.1** 🟦 |
...
> 🧒 **Kid translation of the table:** the top four are like *Mercedes/BMW/Ferrari* — excellent but expensive garages. Nexus is the *DIY smart kit*: it just jumped from ~6.6 to **~7.1/10** — now comfortably ahead of the mid-tier niche players (Cin7, Softeon, Extensiv, HotWax, Brightpearl) and within a whisker of NetSuite & Körber — **and it is still the only one that can be run for the price of a pizza and extended like open-source Lego.**

### 3.1 Honest reading of Nexus's own gaps (from `01-CURRENT-STATE.md`)

> **Updated since the baseline:** backend tests are green (**561 run, 0 failures, 0 errors**), the 6 broken `permission` gates were migrated to resource+action props (server still enforces deny-by-default), and the **mobile RF module shipped** (pick/pack/receive/ship/count/scan screens, camera scanning with a ZXing fallback for browsers without native `BarcodeDetector`, and a server-side scan search endpoint). The **3PL billing + client portal** (per-client rate cards, statement generation, read-only client overview) also shipped and is live-verified, alongside the WES-lite task queue, yard/dock scheduling and the EDI 850/856/810 parser hardening. Those rows are resolved; the table now reflects only the remaining gaps.

| Gap | Impact on these scores | Fix path |
|---|---|---|
| **G3 — AI has no real training data** | AI/ML score capped at 6/10; "AI-first" claim is surface-only today | Curated dataset → real model |
...
| **Mobile RF is young (no production hours, no offline mode, no voice)** | Fulfillment & WMS scores still capped at 7/10 vs 8–9 for mature RF | Offline queue, voice/vision picking, real DC usage |
| **WES orchestration is WES-lite (task queue + commands, no closed-loop robotics)** | WES capped at 6/10 vs 9–10 for Softeon/Körber | Equipment/emulator + slotting loop |
...
> Two former "decisive losses" are now just *young* rather than *absent*: **WES/automation moved 4 → 6/10** (wave plan/release + task queue + automation commands shipped) and **3PL billing moved 4 → 8/10** (rate cards + statements + client portal live-verified). The mobile RF row is no longer a loss either: at **7/10** Nexus beats Cin7 (6), Brightpearl (5) and HotWax (4) on handheld capability — ahead of the enterprise tier (8–9) is a matter of maturity, not presence.

### 5.3 The score story in one number
- Nexus **7.1/10** weighted — up from 6.6 at baseline, driven by the shipped mobile RF module, **3PL billing + client portal (4 → 8)**, **WES-lite task queue/automation (4 → 6)**, **EDI 850/856/810 hardening (7 → 8)**, finance (payments/refunds/credit memos → 7), and the **561-test green suite**. Nexus now **ranks 7th of 12** — ahead of Softeon, Cin7, Extensiv, Brightpearl and HotWax on a like-for-like weighted basis for a mid-market buyer, and within a whisker of NetSuite (7.4) and Körber (7.2).
- The remaining score unlock is still **real AI training data**: with AI moving 6 → 8 (a curated dataset + a real model) and WES going closed-loop, Nexus's weighted total heads toward **≈ 7.6–7.8/10** — genuinely competing with the NetSuite/Körber tier for a fraction of the cost.

### 5.4 What changed since the baseline (August 2026 delta)

| Baseline gap (from `01-CURRENT-STATE.md`) | Status now |
|---|---|
| **G1 — backend tests don't compile** | ✅ **Closed** — 561 tests, 0 failures, 0 errors (new suites: ATP, yard/dock, procurement, invoicing, EDI, BOPIS, billing, rate cards, waves, task queue) |
...
| **Mobile RF app absent (2/10)** | ✅ **Shipped** — RF PWA: Pick, Pack, Receive, Ship, Count, Scan + Home; camera scanning (native `BarcodeDetector`, ZXing fallback); server-side scan search (`GET /rf/search`); deep-linkable screens; E2E-verified (6 checks, 0 console errors) → **7/10** |
...
| **WES = config + IoT hooks only (4/10)** | ✅ **Partially closed** — wave plan/release, task queue, automation commands (honest `simulated:true`); not yet closed-loop robotics → **6/10** |
| **EDI parser fragile** | ✅ **Hardened** — 850/856/810 field-mapping fixes, regex DoS fix, 850 → order creation; 7 unit tests → **8/10** |
| Remaining: AI training data, stub endpoints, rule-based email, RF offline/voice, closed-loop WES | Still open — see §3.1 |
...
2. **✅ Mobile warehouse app — done.** The RF module (pick/pack/receive/ship/count/scan + camera scanning + server-side scan search) shipped and lifted Nexus 6.5 → 6.6. **Next upgrade:** offline mode + picker-assignment UI in the RF client.
3. **✅ 3PL billing + client portal — done.** Per-client rate cards, statement generation and a read-only client overview shipped (4 → 8/10) and lifted Nexus to **7.1/10**, ahead of Cin7/Softeon/Extensiv/HotWax/Brightpearl. **Next upgrade:** self-serve portal (approvals, document upload), credit/payment workflows.
4. **Real ML training data (G3) is the strategic unlock.** AI is Nexus's headline — but it scores only 6/10 because it has no real model. A curated dataset + ONNX runtime moves AI 6→8 and the trust story with it — the single biggest remaining swing on the weighted score.
5. **✅ Test suite (G1), permission gates (G2) and EDI hardening — done.** Backend is green at 561 tests; the EDI parser now maps fields correctly and can't be hung by malformed input. The next cheap, honest win is wiring the stub (`simulated:true`) endpoints to real providers (G4).
...
- **Nexus:** `docs/01-CURRENT-STATE.md` and `docs/00-VISION-DREAM.md` (honest, committed baseline); RF module (pick/pack/receive/ship/count/scan + camera scanning + `GET /rf/search`) committed Aug 2026, E2E-verified; 3PL module (rate cards + billing statements + client portal) committed Aug 2026, live smoke-verified; backend suite at **561 tests / 0 failures**.
